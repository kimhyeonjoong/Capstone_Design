 ...
 */
public class ClothtakuRegisterActivity extends AppCompatActivity {
    private final String TAG = this.getClass().getSimpleName();
    public static ClothInfoItem currentItem = null;

    Context context;

    /**
     * ClothtakuRegisterLocationFragment를 실행하기 위한 기본적인 정보를 설정하고
     * 프래그먼트를 실행한다.
     */
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_clothtaku_register);

        context = this;

        int memberSeq = ((MyApp)getApplication()).getMemberSeq();

        //ClothtakuRegisterLocationFragment로 넘길 기본적인 정보를 저장한다.
        ClothInfoItem infoItem = new ClothInfoItem();
        ...
        //ClothtakuRegisterLocationFragment를 화면에 보여준다.
        GoLib.getInstance().goFragment(getSupportFragmentManager(),
                R.id.content_main, seoultech.capstondesign.clothtaku2.
                        ClothtakuRegisterLocationFragment.newInstance(infoItem));
    }
    ...
    private void setToolbar() {
        ...
        if (actionBar != null) {
            ...
            actionBar.setTitle(R.string.clothtaku_register);
        }
    }
