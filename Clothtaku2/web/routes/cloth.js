...
router.post('/info', function(req, res, next) {
  ...
  var sql_insert = 
    "insert into clothtaku_info (member_seq, name, tel, address, latitude, longitude, description) " +
    "values(?, ?, ?, ?, ?, ?, ?); ";
...
router.post('/info/image', function (req, res) {
  ...
  form.parse(req, function(err, fields, files) {
    var sql_insert = "insert into clothtaku_info_image (info_seq, filename, image_memo) values (?, ?, ?);";
...
router.get('/info/:seq', function(req, res, next) {
  ...
  var sql = 
    ...
    "  if( exists(select * from clothtaku_keep where member_seq = ? and a.seq = info_seq), 'true', 'false') as is_keep, " +
    "  (select filename from clothtaku_info_image where info_seq = a.seq order by seq limit 1) as image_filename " +            
    "from clothtaku_info as a " +
    "where seq = ? ; ";
  console.log("sql : " + sql);
...
router.get('/list', function(req, res, next) {
  ...
  var sql = 
    ...
    "  if( exists(select * from clothtaku_keep where member_seq = ? and info_seq = a.seq), 'true', 'false') as is_keep, " +
    "  (select filename from clothtaku_info_image where info_seq = a.seq) as image_filename " +
    "from clothtaku_info as a " +
    "order by  " + order_add + " " +
    "limit ? , ? ; ";
  console.log("sql : " + sql);
